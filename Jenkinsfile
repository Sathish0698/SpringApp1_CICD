pipeline {
  ...
  stages {
    stage('checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/Sathish0698/SpringApp1_CICD'
   ...
   // Pull docker image from DockerHub and run in EC2 instance 
    ...
    stage('Deploy Docker image to AWS instance') {
	  ...
	  sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker pull sathish98docker/javawebapp'"
          sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker run --name javaApp -d -p 8082:8082 sathish98docker/javawebapp'"
          }
        }
      }
    }
